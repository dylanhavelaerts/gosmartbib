...
import edu.ap.testbackend.repositories.SchoolClassRepository;
import edu.ap.testbackend.repositories.SchoolRepository;
...
public class UserService {
    ...
    // Nog bespreken met klant (hoe lang voor inactive accounts verwijderd mogen worden
     ...
     * - Als een gebruiker al bestaat -> gegevens updaten indien nodig (naam, klassen, ...)
     * - ROL wordt niet overgeschreven aangezien een leerkracht miss de bibbeheerder rol heeft gekregen
     */
    @Transactional
    public UserEntity syncUser(OAuth2User oauth2User) {
        ...
        if (!updatedClasses.equals(user.getClasses())) {
            log.info("Klassen voor gebruiker {} zijn gewijzigd. Oude klassen: {}, Nieuwe klassen: {}", uid, user.getClasses(), updatedClasses);
